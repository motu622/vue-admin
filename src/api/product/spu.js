/* 
包含spu管理相关接口请求函数
*/
import request from '@/utils/request'

export default {
    /* 
    获取销售属性列表
    GET /admin/product/baseSaleAttrList
    */
    getSaleAttrList() {
        return request.get('/vue-admin-template/product/baseSaleAttrList')
    },

    /* 
    删除指定的SPU
    DELETE /admin/product/deleteSpu/{spuId}
    */
    remove(spuId) {
        return request({
            url: '/vue-admin-template/product/spu/deleteSpu',
            method: 'delete',
            params: { spuId }
        })
    },

    /* 
    根据id获取SPU信息
    GET /admin/product/getSpuById/{spuId}
    spuList: 包含多个spu简单信息的数组
    spuInfo: 包含一个spu详细信息的对象
    */
    get(spuId) {
        return request({
            url: '/vue-admin-template/product/getSpuById',
            method: 'get',
            params: { spuId }
        })
    },

    /* 
    添加SPU详情信息
    POST /admin/product/saveSpuInfo
    POST /admin/product/updateSpuInfo
    */
    addUpdate(spuInfo) {
        // return request.post(`/admin/product/${spuInfo.id ? 'update' : 'save'}SpuInfo`, spuInfo)
        return request({
            url: `${spuInfo.id ? '/vue-admin-template/product/updateSpuInfo' : '/vue-admin-template/product/savaSpuInfo'}`,
            method: 'POST',
            data: spuInfo
        })
    },

    /* 
    GET /admin/product/{page}/{limit}
      query参数: category3Id
    */
    getList(page, limit, category1Id, category2Id, category3Id) {
        // return request.get(`/admin/product/${page}/${limit}`, {params: {category3Id}})

        return request({
            url: '/vue-admin-template/product/getSpuList/',
            method: 'GET',
            params: { page, limit, category1Id, category2Id, category3Id }
        })
    },

    findSpuInfoByKeyword(keyword) {
        return request({
            url: `/vue-admin-template/product/sku/findSpuInfoByKeyword`,
            method: 'GET',
            params: { keyword }
        })
    }
}